-- Create sample prompts for sales/revenue operations
-- Note: These will be inserted for the authenticated user when they run this
...
BEGIN
    ...
    IF current_user_id IS NOT NULL THEN
        -- Insert sample prompts with a tag to identify them as samples
        INSERT INTO public.prompts (id, user_id, title, description, purpose, prompt_content) VALUES
        (
            gen_random_uuid(),
            current_user_id,
...
**Ensure all responses maintain a consultative, helpful tone while confidently addressing concerns.**'
        ...
        RAISE NOTICE 'Sample prompts inserted successfully for user %', current_user_id;
    ELSE
        RAISE NOTICE 'No authenticated user found. Sample prompts not inserted.';
    END IF;
END $$;